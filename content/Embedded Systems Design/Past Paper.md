...
> 3. A public method `applyDiscount(double a)` reduces the price of the book by the given percentage and updates the price of the book. Note that this method should use the data member `price` for the calculation using the following equation: $$ \text{price}_\text{new} = \text{price}_\text{old} \times (1-a\%)$$
